...
from networkssemantic.PixelFormerPlus import PixelFormerPlus
...
parser = argparse.ArgumentParser(description='PixelFormerPlus PyTorch implementation.', fromfile_prefix_chars='@')
parser.convert_arg_line_to_args = convert_arg_line_to_args

parser.add_argument('--model_name', type=str, help='model name', default='PixelFormerPlus')
...
if args.dataset == 'kitti' or args.dataset == 'nyu' or args.dataset == 'habitat':
    from dataloaders.dataloader_semantic import NewDataLoader
...
def test(params):
    ...
    model = PixelFormerPlus(version='large07', inv_depth=False, max_depth=args.max_depth)
    ...
    with torch.no_grad():
        for _, sample in enumerate(tqdm(dataloader.data)):
            ...
            semantic_image = Variable(sample['semantic'].cuda())
            # Predict
            depth_est = model(image,semantic_image)
            post_process = True
            if post_process:
                image_flipped = flip_lr(image)
                semantic_image_flipped= flip_lr(semantic_image)
                depth_est_flipped = model(image_flipped,semantic_image_flipped)
    ...
    for s in tqdm(range(num_test_samples)):
        if args.dataset == 'kitti':
            ...
            filename_pred_png = save_name + '/raw/' + scene_name + '_' + lines[s].split()[0].split('/')[3].replace(
                '.jpg', '.png')
            filename_cmap_png = save_name + '/cmap/' + scene_name + '_' + lines[s].split()[0].split('/')[3].replace(
                '.jpg', '.png')
            filename_gt_png = save_name + '/gt/' + scene_name + '_' + lines[s].split()[0].split('/')[3].replace(
                '.jpg', '_gt.png')
        ...
        if args.dataset == 'nyu':
            gt_path = os.path.join(args.gt_path, './' + lines[s].split()[1])
            gt = cv2.imread(gt_path, -1).astype(np.float32) / 1000.0  # Visualization purpose only
            gt[gt == 0] = np.amax(gt)
        elif args.dataset == 'habitat':
            gt_path = os.path.join(args.gt_path, './' + lines[s].split()[1])
            gt = cv2.imread(gt_path, cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255.0*10.0  # Visualization purpose only
            gt[gt == 0] = np.amax(gt)
        ...
        if args.dataset == 'kitti' or args.dataset == 'kittipred':
            ...
            pred_depth_scaled = pred_depth * 255.0/10.0
        else:
        ...
        if args.save_viz:
            ...
            if args.dataset == 'nyu':
                ...
                pred_depth_cropped = pred_depth[45:471, 41:601]
                plt.imsave(filename_cmap_png, (10 - pred_depth_cropped) / 10, cmap='plasma')
            elif args.dataset == 'habitat':
                gt_cropped = gt[45:471, 41:601]
                plt.imsave(filename_gt_png,  (10 - gt_cropped) / 10, cmap='Greys')
                pred_depth_cropped = pred_depth[45:471, 41:601]
                plt.imsave(filename_cmap_png, (10 - pred_depth_cropped) / 10, cmap='Greys')
            else:
                plt.imsave(filename_cmap_png, np.log10(pred_depth), cmap='plasma')
