...
def online_eval(model, dataloader_eval, gpu, ngpus, post_process=False):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        with torch.no_grad():
            ...
            semantic_image = torch.autograd.Variable(eval_sample_batched['semantic'].cuda(gpu, non_blocking=True))
            ...
            pred_depth = model(image,semantic_image)
            if post_process:
                image_flipped = flip_lr(image)
                semantic_image_flipped= flip_lr(semantic_image)
                pred_depth_flipped = model(image_flipped,semantic_image_flipped)
        ...
        if args.garg_crop or args.eigen_crop:
            ...
            if args.garg_crop:
                ...
                elif args.dataset == 'nyu' or args.dataset == 'habitat':
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = PixelFormerPlus(version=args.encoder, inv_depth=False, max_depth=args.max_depth, pretrained=args.pretrain)
    model.train()
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            semantic_image = torch.autograd.Variable(sample_batched['semantic'].cuda(args.gpu, non_blocking=True))

            depth_est = model(image,semantic_image)

            if args.dataset == 'nyu' or args.dataset == 'habitat':
                mask = depth_gt > 0.1
            else:
                mask = depth_gt > 1.0
