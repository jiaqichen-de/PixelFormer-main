...
class BCP(nn.Module):
    """ Bin Center Prediction Module
        ...
        Semanticbin means bin center prediction module assigns a bin partition for each object or according to semantic segmentation
        here there are two bin partitions, foreground and background
        Others means the all pixels share the common bin partition
    """

    def __init__(self, max_depth, min_depth, bcpmode,split,in_features=512, hidden_features=512*4, out_features=128, act_layer=nn.GELU, drop=0.):
        super().__init__()
        ...
        """if we employ transposed convolution to build the bin center prediction module,
           ...
           and keep the second self.pixelbin and add bilinear interpolation.
           ...
           the third self.pixelbin and comment the other self.pixelbin out"""
    ...
    def forward(self, x):
        ...
        elif self.bcpmode== "Semanticbin":
          
          x_foreground= torch.mean(x.flatten(start_dim=2), dim = 2)
          x_background= torch.mean(x.flatten(start_dim=2), dim = 2)
          x_foreground = self.fc1(x_foreground)
          x_foreground = self.act(x_foreground)
          x_foreground = self.drop(x_foreground)
          x_foreground = self.fc2(x_foreground)
          x_foreground = self.drop(x_foreground)
          bins_foreground = torch.softmax(x_foreground, dim=1)
          bins_foreground = bins_foreground / bins_foreground.sum(dim=1, keepdim=True)
          bin_widths_foreground = (self.max_depth - self.min_depth) * bins_foreground         
          bin_widths_foreground = nn.functional.pad(bin_widths_foreground, (1,0), mode='constant', value=self.min_depth)          
          bin_edges_foreground = torch.cumsum(bin_widths_foreground, dim=1)         
          centers_foreground = 0.5 * (bin_edges_foreground[:, :-1] + bin_edges_foreground[:, 1:])
          
          n, dout = centers_foreground.size()
          centers_foreground = centers_foreground.contiguous().view(n, dout, 1, 1)

          x_background = self.fc3(x_background)
          x_background = self.act(x_background)
          x_background = self.drop(x_background)
          x_background = self.fc4(x_background)
          x_background = self.drop(x_background)
          bins_background = torch.softmax(x_background, dim=1)
          bins_background = bins_background / bins_background.sum(dim=1, keepdim=True)
          bin_widths_background = (self.max_depth - self.min_depth) * bins_background        
          bin_widths_background = nn.functional.pad(bin_widths_background, (1,0), mode='constant', value=self.min_depth)          
          bin_edges_background = torch.cumsum(bin_widths_background, dim=1)         
          centers_background = 0.5 * (bin_edges_background[:, :-1] + bin_edges_background[:, 1:])
          
          n, dout = centers_background.size()
          centers_background = centers_background.contiguous().view(n, dout, 1, 1)
          return centers_foreground, centers_background
        else:
...
class PixelFormerPlus(nn.Module):
    ...
    def __init__(self, version=None, inv_depth=False, pretrained=None, 
        ...
        bcp_mode= "Semanticbin"
        
        if bcp_mode== "Pixelbin":
           self.bcp_mode="BinAdjustment"
        elif bcp_mode== "Semanticbin":
           self.bcp_mode="Semanticbin"
        else:
           self.bcp_mode="None"
        ...
        self.SemanticCombination=SemanticCombination(input_dim=128)
    ...
    def forward(self, imgs,semantic_img):
        ...
        #Linear combination
          ...
          elif self.bcp_mode=="Semanticbin":
               bin_centers_foreground, bin_centers_background = self.bcp(q4)
               f=self.SemanticCombination(semantic_img, f, q0, bin_centers_foreground, bin_centers_background)
...
class DispHead(nn.Module):
    ...
    def forward(self, x, centers):
        if self.bcpmode == "MultiHead":
          ...
          return x
class SemanticCombination(nn.Module):
    """Linear Combination of bin Centers and generated probability values
        There are two bin partitions
        Bins for Foreground and Bins for Background"""
    def __init__(self, input_dim):
        super().__init__()
        self.input_dim = input_dim
        self.upsample=Upsample(upsample_mode="bilinear",input_channels=1)
        self.conditional_log_binomial = ConditionalLogBinomial(
            self.input_dim, self.input_dim, n_classes=128, min_temp=5, max_temp=50)
    def forward(self, semantic_img, x, d0,bin_centers_foreground, bin_centers_background):
           
          
         x=self.upsample(x)
         d0 =self.upsample(d0)
         x=self.conditional_log_binomial(x, d0)
         x_foreground = x
         x_background = x
         mask_foreground=torch.zeros_like(x_foreground)
         #There are six objects on segmentation images
         #1,2,3,4,5,6 belong to foreground
         #0 belongs to background
         
         _, C, _, _ = x.size()
         semantic_img=torch.tile(semantic_img,(1,C,1,1))
         mask_foreground[semantic_img==1] = 1
         mask_foreground[semantic_img==2] = 1
         mask_foreground[semantic_img==3] = 1
         mask_foreground[semantic_img==4] = 1
         mask_foreground[semantic_img==5] = 1
         mask_foreground[semantic_img==6] = 1

         x_foreground=x_foreground*mask_foreground
         mask_background=torch.zeros_like(x_background)
         mask_background[semantic_img==0] = 1
         mask_background[semantic_img>6] = 1

         x_background=x_background*mask_background
         x = torch.sum(x_foreground * bin_centers_foreground, dim=1, keepdim=True)
         x=x+torch.sum(x_background * bin_centers_background, dim=1, keepdim=True)
