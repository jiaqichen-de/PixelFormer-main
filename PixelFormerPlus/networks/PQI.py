...
Those classes are the implementation of DeepLabv3+
"""
...
"""ASPP pixel query initialiser module consists of pooling operation
a 1x1 convolution layer and three 3x3 atrous convolution layers with rates=(6,12,18), 
each accompanied by batch normalization (BN) and ReLU activation"""
"""Args:
        
        in_channels (int): Input channels.
        out_channels (int): Output channels.
        rate(int): Dilation of Atrous convolution
        channels (int): Channels after modules, before conv_seg.
        conv_cfg (dict|None): Config of conv layers.
        norm_cfg (dict|None): Config of norm layers.
        act_cfg (dict): Config of activation layers.
...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels,atrous_rates):
        ...
        self.project = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(0.1),)
    """a convolution operation is applied to reduce the dimensionality."""
...
class PSP(BaseDecodeHead):
    ...
    def __init__(self, pool_scales=(1, 2, 3, 6), **kwargs,):
        ...
        self.bottleneck = ConvModule(
            ...
            act_cfg=self.act_cfg)
        #add ASPP mode
        #when set ASPP mode, and ASPP will replace SPP
        self.EncoderGlobal_mode= "ASPP"
        self.aspp=ASPP(self.in_channels[-1],
            self.channels,(6,12,18))

    def psp_forward(self, inputs):
        """Forward function of PSP module."""
        x = inputs[-1]
        if self.EncoderGlobal_mode=="ASPP":
           output=self.aspp(x)
        else:
           psp_outs = [x]
